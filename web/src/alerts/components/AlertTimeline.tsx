      ...
      {ordered.map((revision) => {
        ...
        return (
          <li key={revision.revision_id}>
            <article>
              <header>
                <div><span>Version {String(revision.revision_number)}</span><h3>{headline || (revision.is_deleted ? 'Removal recorded' : 'No headline provided')}</h3></div>
                {revision.is_deleted && <strong className="alert-feature__deleted">Removed</strong>}
              </header>
              <p className="alert-timeline__range">Seen {formatMelbourneTime(revision.revision_first_seen_at)} to {formatMelbourneTime(revision.revision_last_seen_at)}</p>
              <details>
                <summary>Version details</summary>
                <div className="alert-timeline__content">
                  {description ? <p className="alert-feature__source-copy">{description}</p> : <p>No description provided in this version.</p>}
