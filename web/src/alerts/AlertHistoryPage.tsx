...
export function AlertHistoryPage() {
  ...
  const rangeError = formError ?? (!bookmarkedFiltersValid ? 'Some filters in this link are invalid. Clear them and try again.' :
    !fromValid || !toValid ? 'Choose valid start and end dates.' : !ordered ? 'Start date must be before end date.' : null)
  const input = {
  ...
  const applyFilters = (values: HistoryFilterValues) => {
    ...
    if ((values.from && !from) || (values.to && !to)) return setFormError('Enter valid start and end dates.')
    if (from && to && new Date(from).getTime() >= new Date(to).getTime()) return setFormError('Start date must be before end date.')
    setFormError(null)
  ...
  return (
    <main id="main" className="workspace feature-page alert-history-page">
      <section className="alert-feature__intro" aria-labelledby="alert-history-title">
        ...
        <p>Past alerts are no longer included in the latest service updates. We do not label alerts as planned or unplanned.</p>
      </section>
      ...
      <section className="alert-history-results" aria-labelledby="alert-history-results-title">
        <header><div><span>Results</span><h2 id="alert-history-results-title">Past alerts</h2></div>{history.data && <strong>{formatCount(history.data.meta.total)} total</strong>}</header>
        ...
        {history.error && !history.data && <ErrorState title="Alert history could not be loaded" message="Try again in a moment." onRetry={() => void history.refetch()} />}
        {history.error && history.data && <ErrorState cached title="Could not refresh alert history" message="Showing the last available results." onRetry={() => void history.refetch()} />}
        {history.data?.data.length === 0 && <div className="empty-state"><strong>No past alerts found</strong><p>Try a wider date range or remove a filter.</p></div>}
