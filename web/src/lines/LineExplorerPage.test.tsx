...
describe('line explorer routes', () => {
  ...
  it('renders official safe colors, counts, episode terminology, and raw-ID links', async () => {
    ...
    expect(within(card as HTMLElement).getByText('Alerts recorded, last 30 days')).toBeVisible()
    ...
    const analytics = within(card as HTMLElement).getByRole('link', { name: 'Alert analytics' })
  ...
  it('shows empty and independent error states', async () => {
    ...
    expect(await screen.findByText('Past alert counts are unavailable')).toBeVisible()
  })
  ...
  it('preserves colon and slash IDs in detail requests and renders strict associations', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Past alerts').nextElementSibling).toHaveTextContent('18'))
    ...
    expect(screen.getByText(/may not include every affected service/i)).toBeVisible()
