...
func (c Client) FetchAlerts(ctx context.Context) (FetchResult, error) {
	...
	if err != nil {
		return FetchResult{}, fmt.Errorf("create service-alert request: %w", err)
	}
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		return FetchResult{}, fmt.Errorf("fetch service alerts: HTTP %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
	...
	if err != nil {
		return FetchResult{}, fmt.Errorf("read service-alert response: %w", err)
	}
	if len(body) > maxPayloadBytes {
		return FetchResult{}, fmt.Errorf("service-alert response exceeds %d bytes", maxPayloadBytes)
	}
	if len(body) == 0 {
		return FetchResult{}, fmt.Errorf("service-alert response is empty")
	}
